...
layout: post
type: project
title: Code snippets for pytorch
subtitle: "Some of the most useful code for pytorch"
comments: true
---

I am switching to pytorch and here are some piece of code that I think will be
useful to copy-paste later.
...
Pytorch handles data quite cleanly. Data is loaded as tensors and then 
iterated using an iterator. 
...
In the example above, the three iterators `train_loader`, `valid_loader`, 
and `test_loader` iterate through all pairs of (data, target)s. The training
process for some given `model`, `optimizer`, and `criterion`:

```python
# number of epochs to train the model
n_epochs = 8 
# track change in validaiton loss
...
for epoch in range(1, n_epochs+1):
    ...
    for data, target in train_loader:
        ...
        # clear the gradients of all optimized variables because by default
        # optimizer accumulates gradients after every batch. This design is
        # convenient for recurrent neural networks
        optimizer.zero_grad()
    ...
    if valid_loss <= valid_loss_min:
        ...
        valid_loss_min,
        valid_loss))
        torch.save(model.state_dict(), 'model_cifar.pt') # state_dict() is a function call 
