...
RNN where each hidden unit is parameterized by a high rank tensor (≥2)**.
    ...
    the hidden tensor $$H_{t+L-1}$$. As in the figure above, $$y_t$$ is computed from $$h^3_{t-1}$$ - the last 
    layer of the hidden unit $$H_{t-1}$$. Also, the implicit depth P = L = 3 leads us to the same computation.  
...
In here, $$A_t$$ is the activation tensor. $$H^\text{cat}_{t-1}%_$$ represents the concatination of a hidden unit's 
